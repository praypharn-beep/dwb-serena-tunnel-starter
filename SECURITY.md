...
- **Resilience settings:** the task is persisted with `StartWhenAvailable`, restart-on-failure (10 attempts at 1-minute intervals), unlimited execution time, `IgnoreNew` multiple-instance behavior, battery-safe continuation, and **idle-end termination disabled**. These settings are part of the installer contract rather than machine-local manual remediation.
...
- the supervisor's **`tunnel-client.exe` child** — matched on the exact resolved `tunnel-client.exe` path for this checkout plus a command line containing `run --profile dwb-serena`. Its PID is persisted separately in `dwb-serena-tunnel-client.pid` by the supervisor itself and is also discoverable live while the supervisor is running. The separate PID exists specifically so an externally terminated supervisor cannot make a still-listening orphan client unmanageable; the client is still identity-verified before any stop signal is sent.

Both identities matter because `tunnel-client.exe` is a genuine OS child of the supervisor, and **Windows does not terminate a process's children when the parent process is killed**. When the supervisor is still alive, `stop` always stops it first (halting its restart loop) and then stops the identity-verified child. If the supervisor is already gone, `stop` falls back to the separately persisted `dwb-serena-tunnel-client.pid`, verifies that live PID against the exact executable path and command line, and only then stops the orphan client. This avoids relying on a tree-kill from an already-dead parent PID and preserves the same PID-reuse safety boundary in both normal and orphan-recovery paths.
